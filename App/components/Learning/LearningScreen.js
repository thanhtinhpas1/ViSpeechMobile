...
import { Image, ScrollView, StyleSheet, Text, TextInput, View, PermissionsAndroid, Alert } from "react-native";
...
export default LearningScreen = ({ levelLesson, currentQuestionIndex }) => {
    ...
    const onPressNext = () => {
        ...
        if (levelLesson.questions[index]) {
            Actions.learning({ levelLesson, currentQuestionIndex: index })
        } else if (levelLesson.questions.length === index) {
            Actions.completeLearning({ levelLesson })
        }
    }
